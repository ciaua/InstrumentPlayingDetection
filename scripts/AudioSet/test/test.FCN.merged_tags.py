...
class Net(nn.Module):
    def __init__(self, num_labels, feat_dim, feat_mean, feat_std):
        ...
        self.conv1 = nn.Conv2d(
            feat_dim, 256, kernel_size=(5, 1), padding=(2, 0))
        self.bn1 = nn.BatchNorm2d(256)

        self.conv2 = nn.Conv2d(
            256, 256, kernel_size=(5, 1), padding=(2, 0))
        self.bn2 = nn.BatchNorm2d(256)

        self.fconv1 = nn.Conv2d(256, 512, kernel_size=1)
...
if __name__ == '__main__':
    ...
    # Default setting
    feat_type_list = ["logmelspec10000.16000_2048_512_128.0.raw"]
    # n_top = info['num_tags']
    ...
    # Scaler dir
    tr_data_dir = os.path.join(
        tr_base_dir,
        'exp_data.audio.target_time.{}'.format(clip_limit))

    scaler_fp_list = [
        os.path.join(tr_data_dir, feat_type, 'scaler.pkl')
        for feat_type in feat_type_list]
    ...
    for fn in fn_list:
        ...
        feat_list = [np.load(feat_fp) for feat_fp in feat_fp_list]
        feat_list = [
