...
class Net(nn.Module):
    def __init__(self, num_labels, feat_dim, scaler_list):
        ...
        self.fconv1 = nn.Conv2d(
            256*num_types, 256*num_labels, kernel_size=1)
        self.fbn1 = nn.BatchNorm2d(256*num_labels)
        self.fconv2 = nn.Conv2d(
            256*num_labels, 256*num_labels, kernel_size=1, groups=num_labels)
        self.fbn2 = nn.BatchNorm2d(256*num_labels)

        self.fconv = nn.Conv2d(
            256*num_labels, self.num_labels, kernel_size=1, groups=num_labels)
