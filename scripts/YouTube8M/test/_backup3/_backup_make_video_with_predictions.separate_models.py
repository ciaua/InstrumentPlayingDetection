...
def make_structure_image(input_var, num_tags):
    ...
    # Late conv
    net['conv.fc6'] = ConvLayer(net['pool5'],
                                num_filters=4096,
                                filter_size=7,
                                pad=3)  # change to pad=3 when testing
    net['conv.drop6'] = DropoutLayer(net['conv.fc6'],
                                     p=0.5)
    net['conv.fc7'] = ConvLayer(net['conv.drop6'],
                                num_filters=2048,
                                filter_size=1)
    net['conv.drop7'] = DropoutLayer(net['conv.fc7'],
                                     p=0.5)
    net['conv.fc8'] = ConvLayer(net['conv.drop7'],
                                num_filters=num_tags,
                                filter_size=1,
                                nonlinearity=sigmoid)
    net['pooled_output'] = GlobalPoolLayer(net['conv.fc8'],
                                           pool_function=T.max)
...
def add_audio(out_fp, video_fp, audio_fp, time_range):
    ...
    vid.write_videofile(out_fp, codec='libx264', temp_audiofile='temp.mp3')


if __name__ == '__main__':

    # song_id = '8dcXdTOEhpQ'  # guitar, accordion
    # song_id = '7gzeVfymLvA'  # accordion
    # song_id = '4crxP6Ti5f0'  # flute
    # song_id = '0ly4HoD6kXo'  # piano
    # song_id = '9DhAj5NdKtg'  # cello
    # song_id = '-QQKkiRjpYM'  # violin
    # song_id = '-e4ck4lx7ak'  # piano
    # song_id = '1mCi6eWXVbg'  # accordion
    # song_id = '1uOpBj3pt4c'  # flute, not good
    # song_id = 'KmeVCUPeZ8s'  # violin
    # song_id = '0JClF4KPfC4'  # drum
    ...
    # song_id = '-AJgf-66PzA'  # drummer, very good example
    # song_id = '-R0267o4lLk'  # drummer, very good example
    # song_id = '4osUq26BCDU'  # piano
    # song_id = '5nJAyKF7iWo'  # piano
    # song_id = '-BusjZuP9KQ'  # piano
    # song_id = '23sQgYKobrs'  # accordion
    # song_id = '5ueksFEtkG0'  # accordion
    # song_id = '-N3SXu15iDk'  # piano
    song_id = '-HdsC1RH8-Y'  # guitar
    # song_id = '-6y0paEQI6s'  # guitar
    # song_id = '-5rwEwMQ1es'  # guitar
    # song_id = 'EwV2CFTzX1Q'  # cello
    # song_id = 'HwaE5s1dQxI'  # cello
    ...
    out_dir = os.path.join(base_out_dir, '{}_{}'.format(model_id_i, model_id_o))
    ...
    network_i = net_dict_i['conv.fc8']
    network_o = net_dict_o['conv.fc8']
    ...
    for one_image, one_dof in zip(images[:num_frames], dof[:num_frames]):
        pred_one_i, pred_one_o = func_pr(one_image[None, :], one_dof[None, :])
    ...
    # out_fp_video_only = os.path.join('.', '{}.video_only.mp4'.format(song_id))
    out_fp_video_only = os.path.join('.', 'temp.mp4')
    out_fp = os.path.join(out_dir, '{}.{}.mp4'.format(song_id, test_instrument))

    pred_fps = sr/(hop*num_frames_per_seg)
