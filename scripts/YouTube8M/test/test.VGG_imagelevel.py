...
class Net(nn.Module):
    def __init__(self, num_labels, feat_dim, feat_mean):
        ...
        # Basic
        self.mean = feat_mean[None, :, None, None]
        ...
        self.lrn1 = LRN(5, alpha=1e-4, k=1)
        ...
        self.conv6 = nn.Conv2d(512, 4096, kernel_size=6, stride=1, padding=3)
        self.conv7 = nn.Conv2d(4096, 2048, kernel_size=1, stride=1, padding=0)
        self.conv8 = nn.Conv2d(
            2048, num_labels, kernel_size=1, stride=1, padding=0)
...
if __name__ == '__main__':
    # Settings
    num_tags = 9
    imagenet_idx_list = [401, 486, 541, 558, 402, 579, 776, 513, 889]

    model_id_i = 'VGG_CNN_M_2048'
    ...
    param_values = pmodel['param_values']
    ...
    mean_image = Variable(torch.FloatTensor(mean_image)).cuda()
    net = Net(1000, feat_dim=3, feat_mean=mean_image)
    net.load_state_dict(param_values)
    net.eval()
    net.cuda()
    ...
    for inst in inst_list:
        ...
        inst_idx = imagenet_idx_list[inst_list.index(inst)]
        ...
        for fn in fn_list:
            ...
            try:
                ...
                print(repr(e))
                one_image = one_image.cpu()

                net.cpu()
                net.mean = net.mean.cpu()
                pred_one, _ = net(one_image[None, :])
                net.cuda()
                net.mean = net.mean.cuda()

            pred_one = pred_one.data.cpu().numpy()
            pred_one = pred_one[0, imagenet_idx_list]
            pred_ = pred_one.max(1).max(1)
            print(pred_)
            raw_input(123)
