...
if __name__ == '__main__':
    ...
    # test_measure_type = 'recall_score_one'
    test_measure_type = 'precision_score_one'

    threshold = 0.5

    num_train_tags = 9
    num_top_test_tags = 12
    ...
    out_dir = os.path.join(base_out_dir, 'test_result.frame.auc')
    ...
    for fn in fn_list:
        ...
        # Predict and upscale
        prediction_ = func_pr(*feat_list)
        # prediction = prediction[0, :, :, 0].T
        prediction_ = prediction_[0].T
        prediction_ = (prediction_ > threshold).astype(int)
        ...
        try:
            ...
            pred_all = np.concatenate([pred_all, sub_prediction], axis=0)
        except:
            anno_all = anno
            pred_all = sub_prediction
