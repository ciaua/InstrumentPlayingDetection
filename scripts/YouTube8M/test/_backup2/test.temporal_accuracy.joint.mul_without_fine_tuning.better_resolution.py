...
if __name__ == '__main__':
    ...
    model_id_i = '20170325_223405'  # image
    model_id_o = '20170324_194642'  # dense optical flow
    ...
    base_pred_dir_i = os.path.join(
        base_dir,
        'prediction.rgb_image.{}_{}.fragment_{}s'.format(
            time_range[0], time_range[1], fragment_unit),
        model_id_i)
    base_pred_dir_o = os.path.join(
        base_dir,
        'prediction.dense_optical_flow.{}_{}.fragment_{}s'.format(
            time_range[0], time_range[1], fragment_unit),
        model_id_o)
    ...
    out_dir = os.path.join(
        ...
        'mul_without_fine_tuning.{}__{}'.format(model_id_i, model_id_o))
    ...
    for uu, song_id in enumerate(all_id_list):
        ...
        for fn in test_fn_list:
            #
            pred_fp_i = os.path.join(pred_dir_i, '{}.npy'.format(fn))
            pred_fp_o = os.path.join(pred_dir_o, '{}.npy'.format(fn))
            pred_i = np.load(pred_fp_i)
            pred_o = np.load(pred_fp_o)

            num_segments = pred_i.shape[0]

            pred = (pred_i*pred_o).max(axis=2).max(axis=2)
