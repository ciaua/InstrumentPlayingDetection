...
if __name__ == '__main__':
    ...
    audioanno_type = 'binary_temporal_instrument'
    ...
    base_audioanno_dir = os.path.join(
        ...
        '{}.threshold_{}.{}_{}.{}'.format(
            audioanno_type, threshold_str, sr, hop_size, audioanno_model_id)
    )
    ...
    out_dir = os.path.join(
        ...
        'audioanno_feats.threshold_{}.{}.{}.plus128'.format(
            threshold_str, audioanno_model_id, out_feat_type))
    ...
    for phase in ['tr', 'va', 'te']:
        ...
        for ii, _id in enumerate(id_list):
            ...
            out_fp_list = list()
            for fn in fn_list:
                audioanno_fp = os.path.join(audioanno_dir, fn)

                img_feat_fp = os.path.join(img_feat_dir, fn)
                dof_feat_fp = os.path.join(dof_feat_dir,
                                           fn.replace('.npy', '.png'))

                fps = [audioanno_fp, img_feat_fp, dof_feat_fp]
