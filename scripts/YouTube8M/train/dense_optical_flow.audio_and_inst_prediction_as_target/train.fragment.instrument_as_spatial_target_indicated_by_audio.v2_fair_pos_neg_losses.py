...
def validate(net, X_va_list, y_va, audio_threshold, inst_threshold,
    ...
    for i_batch, batch_data in enumerate(iterator_va):
        ...
        try:
            ...
            pos_s = pos_target.sum()+1.
            neg_s = neg_target.sum()+1.

            loss = criterion(clamp(x*pos_target),
                             artificial_target*pos_target)/pos_s + \
                criterion(clamp(x*neg_target),
                          artificial_target*neg_target)/neg_s
        except Exception:
            # print(repr(e))
            h0, w0 = x.size()[2:]
            ...
            pos_target = artificial_target
            neg_target = 1.-artificial_target
            pos_s = pos_target.sum()+1e-6
            neg_s = neg_target.sum()+1e-6
            loss = criterion(clamp(x*pos_target),
                             artificial_target*pos_target)/pos_s + \
                criterion(clamp(x*neg_target),
                          artificial_target*neg_target)/neg_s
