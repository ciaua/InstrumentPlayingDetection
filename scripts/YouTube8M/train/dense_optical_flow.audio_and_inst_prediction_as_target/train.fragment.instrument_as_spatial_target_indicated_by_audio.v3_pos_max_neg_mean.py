...
def validate(net, X_va_list, y_va, audio_threshold, inst_threshold,
    ...
    for i_batch, batch_data in enumerate(iterator_va):
        ...
        # Loss
        try:
            pos_target = artificial_target
            neg_target = 1.-artificial_target
            # pos_s = torch.max(pos_target.sum(), 1.)
            # neg_s = torch.max(neg_target.sum(), 1.)
            # neg_s, _ = torch.max(neg_target.sum(), torch.ones(1).cuda())
            neg_s = neg_target.sum()+1.

            loss_neg = criterion(
                clamp(x*neg_target), artificial_target*neg_target)/neg_s

            x_pos_max = F.max_pool2d(x*pos_target, pos_target.size()[2:])
            target_pos_max = F.max_pool2d(
                artificial_target*pos_target, pos_target.size()[2:])

            loss_pos = criterion_mean(clamp(x_pos_max), target_pos_max)
            ...
            w = min(w0, w1)
            x = x[:, :, :h, :w]
            artificial_target = artificial_target[:, :, :h, :w]
...
if __name__ == '__main__':
    ...
    criterion_mean = nn.BCELoss().cuda()
    ...
    for epoch in range(start_epoch, 1+num_epochs):
        ...
        for i_batch, batch_data in enumerate(iterator_tr):
            ...
            # Loss
            # pos_weight = pos_target/pos_target.sum().expand_as(pos_target)
            # neg_weight = neg_target/neg_target.sum().expand_as(neg_target)
            ...
            try:
                ...
                # pos_s = torch.max(pos_target.sum(), 1.)
                # neg_s = torch.max(neg_target.sum(), 1.)
                # neg_s = torch.max(neg_target.sum(), torch.ones(1).cuda())
                neg_s = neg_target.sum()+1.
