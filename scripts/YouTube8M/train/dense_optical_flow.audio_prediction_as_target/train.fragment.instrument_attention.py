...
def validate(net, X_va_list, y_va, threshold, measure_func):
    ...
    for i_batch, batch_data in enumerate(iterator_va):
        ...
        inst = Variable(inst, volatile=True)

        x, x_pooled = net(feat, audio, inst)
...
class Net(nn.Module):
    ...
    def forward(self, x, audio, inst):
        ...
        # print(x.size(), audio.size(), inst.size())
        h0, w0 = x.size()[2:]
        h1, w1 = inst.size()[2:]
        h = min(h0, h1)
        w = min(w0, w1)
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, 1+num_epochs):
        ...
        for i_batch, batch_data in enumerate(iterator_tr):
            ...
            target = (audio > audio_threshold).float()
        ...
        score_va, mean_loss_va = validate(
            net, X_va_list, y_va, audio_threshold, measure_func)

        # ### Testing ###
        print('')
        print('Testing...')
        score_te = test(net, X_te_list, y_te, audio_threshold, measure_func)
