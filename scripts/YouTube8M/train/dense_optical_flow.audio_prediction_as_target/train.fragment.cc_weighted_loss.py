...
def compute_cc(aa, bb, epsilon=1e-6):
    """
    ...
    """
    aa = aa-aa.mean(0).expand_as(aa)
    bb = bb-bb.mean(0).expand_as(bb)
    # print(aa.size(), bb.size())
    a_norm = (aa*aa).sum(0)**0.5
    b_norm = (bb*bb).sum(0)**0.5

    r = (aa*bb).sum(0)/(a_norm*b_norm+epsilon)
    r = r.view(r.size(1))
...
def validate(net, X_va_list, y_va, threshold, measure_func):
    ...
    for i_batch, batch_data in enumerate(iterator_va):
        ...
        feat = batch_data[0][0]
        target_raw = batch_data[1]
        target = (target_raw > threshold).float()
        ...
        cc = F.relu(compute_cc(Variable(target_raw).cuda(), x_pooled).max())

        loss = cc*criterion(x_pooled, target)
        loss_va = loss.data[0]
