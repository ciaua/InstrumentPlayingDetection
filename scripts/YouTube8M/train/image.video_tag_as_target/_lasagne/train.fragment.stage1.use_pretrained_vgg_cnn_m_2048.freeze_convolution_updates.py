...
def freeze_all(network):
    layer_list = layers.get_all_layers(network)
    for layer in layer_list:
        freeze(layer)


def make_structure(input_var, mean_image, num_tags):
...
if __name__ == '__main__':
    ...
    pretrained_model = it.unpickle(pretrained_model_fp)

    # shape=(224, 224, 3)
    mean_image = pretrained_model['mean_image'].mean(axis=0).mean(axis=0)
    pretrained_param_values = pretrained_model['values']
    ...
    net_dict = make_structure(input_var, mean_image, num_tags)
    network = net_dict['pooled_output']

    # Freeze convolutions
    layer_conv5 = net_dict['conv5']
    freeze_all(layer_conv5)

    # Load pretrained params to conv5
    num_params_to_conv5 = len(layers.get_all_param_values(layer_conv5))
    layers.set_all_param_values(layer_conv5,
                                pretrained_param_values[:num_params_to_conv5])
    ...
    #    resize_ratio_range=resize_ratio_range
