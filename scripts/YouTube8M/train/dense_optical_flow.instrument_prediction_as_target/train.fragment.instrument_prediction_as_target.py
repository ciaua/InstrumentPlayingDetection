...
def validate(net, X_va_list, y_va, inst_threshold, measure_func):
    ...
    for i_batch, batch_data in enumerate(iterator_va):
        ...
        feat = batch_data[0][0]
        inst = batch_data[0][1]
        vid_target = batch_data[1]

        target = (inst > inst_threshold).float()
        ...
        loss = criterion(x[:, :, :h, :w], target[:, :, :h, :w])
        loss_va = loss.data[0]

        target_va_list.append(vid_target.cpu().numpy())
...
def test(net, X_te_list, y_te, inst_threshold, measure_func):
    ...
    for i_batch, batch_data in enumerate(iterator_te):
        ...
        target_te_list.append(vid_target.cpu().numpy())
...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # Pool with instrument prediction
        # raw_input(123)
...
if __name__ == '__main__':
    model_type = 'fcn_action_instrument-target'
    ...
    anno_feats_dir = os.path.join(
        ...
        'anno_feats_inst.inst_{}.{}.fill_{}.plus128'.format(
            inst_model_id, feat_type, fill_factor))
    ...
    for epoch in range(start_epoch, 1+num_epochs):
        ...
        score_va, mean_loss_va = validate(
            net, X_va_list, y_va, inst_threshold, measure_func)

        # ### Testing ###
        print('')
        print('Testing...')
        score_te = test(
            net, X_te_list, y_te, inst_threshold, measure_func)
