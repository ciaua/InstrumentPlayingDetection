...
def extract_one(vid,
                ...
                num_frames_per_seg, num_flows_per_frame, fps):
    # resize
    ...
    fill_factor = hop/fps
    finer_frames = np.stack(list(vid.iter_frames(fps))).astype('uint8')
    ...
    finer_frames = np.pad(
        finer_frames,
        pad_width=((half_num_flows_per_frame, half_num_flows_per_frame),
    ...
    stacked_flow_all = np.minimum(np.maximum(stacked_flow_all + 127, 0), 255)
...
if __name__ == '__main__':
    ...
    frame_per_sec = 32  # temporal resolution of the frame dense optical flows
    ...
    for fn in fn_list:
        args = (base_out_dir, vid_dir, fn, vid_ext,
                sr, hop, time_range, target_size,
                num_frames_per_seg, num_flows_per_frame, frame_per_sec,
