...
def save_as_png(feat, fn, out_dir, old_shape, new_shape):
        ...
        if tuple(output.shape[2:]) != tuple(target_size):
            print(
                'Wrong Size: {}. {}, {}'.format(fn,
                                                ...
                                                tuple(target_size)))
            return
        save_as_png(output, fn, out_dir, old_shape, new_shape)
...
if __name__ == '__main__':
    ...
    num_flows_per_frame = 3
    time_range = (30, 60)

    old_shape = (59, 2*num_flows_per_frame, 256, 256)
    new_shape = (np.prod(old_shape[:2]), np.prod(old_shape[2:]))
    target_size = np.array(old_shape[2:])  # (height, width)
