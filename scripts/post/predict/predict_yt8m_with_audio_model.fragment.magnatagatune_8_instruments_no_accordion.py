...
def do_one(args):
    fn, base_feat_dir_list, base_out_dir, net, fragment_unit, label_idx_list = \
        args
    print(fn)
    ...
    for subfn in subfn_list:
        ...
        try:
            ...
            # print(x.size())
            pred = x.data.cpu().numpy()[0, label_idx_list, :].T
            # print(pred.shape)
            # raw_input(123)
            np.save(out_fp, pred)
...
class Net(nn.Module):
    def __init__(self, num_labels, feat_dim, scaler_list):
        ...
        self.conv1 = nn.Conv2d(
            feat_dim*num_types, 32*num_types,
            kernel_size=(8, 1), padding=(4, 0), groups=num_types)
        self.conv2 = nn.Conv2d(
            32*num_types, 32*num_types,
            kernel_size=(8, 1), padding=(4, 0), groups=num_types)

        self.fconv1 = nn.Conv2d(32*num_types, 512, kernel_size=1)
    ...
    def forward(self, x_list):
        ...
        # Late
        x = self.dropout(F.relu(self.fconv1(x)))
        x = self.dropout(F.relu(self.fconv2(x)))
...
if __name__ == '__main__':
    ...
    model_tag_fp = os.path.join(base_data_dir, 'labels.top188.txt')
    ...
    model_dir = "/home/ciaua/NAS/home/data/AudioSet/pretrained_models/"
    param_fp = os.path.join(
        model_dir, 'FCN.clip2frame.MagnaTagATune.{}.torch'.format(model_id))
    param_dict = torch.load(param_fp)
    # ###

    audio_data_dir = '/home/ciaua/NAS/home/data/AudioSet/'
    ...
    # Load the scaler
    scaler_list = param_dict['scaler_list']
    ...
    # Load param
    state_dict = param_dict['param_values']
